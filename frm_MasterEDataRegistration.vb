...
Public Class frm_MasterEDataRegistration
    ...
    Private Sub frm_MasterEDataRegistration_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        If (dgv_MasterEDataRegistration.Rows.Count > 0) Then
            dgv_MasterEDataRegistration.Rows(0).Selected = True
        End If
    End Sub

    Private Sub frm_MasterEDataRegistration_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        frm_MasterEDataRegistration_Resize(Me, Nothing)
        txt_CariData.Focus()
        Try
            cn.ConnectionString = "Provider=SQLNCLI11;Server=192.168.0.1;Database=AN_SUMATRA;Uid=itdevelopment;Pwd=itdevelopment2015"
            cn.Open()
            rs = New ADODB.Recordset
            strs = "SELECT * FROM  [AN_SUMATRA].[dbo].[TM_tb_Employees] order by [id]"
            ...
            End With
            Me.dgv_MasterEDataRegistration.DataSource = RecordSetToDataTable(rs)
            Call tampilgrid()
    ...
    End Sub
    Sub tampilgrid()
        dgv_MasterEDataRegistration.Columns(0).Width = 40
        dgv_MasterEDataRegistration.Columns(1).Width = 100
        dgv_MasterEDataRegistration.Columns(2).Width = 150
        dgv_MasterEDataRegistration.Columns(3).Width = 150
        dgv_MasterEDataRegistration.Columns(4).Width = 100
        dgv_MasterEDataRegistration.Columns(5).Width = 70
        dgv_MasterEDataRegistration.Columns(6).Width = 60
        dgv_MasterEDataRegistration.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.None
        dgv_MasterEDataRegistration.RowTemplate.Height = 17
    ...
    Private Sub frm_MasterEDataRegistration_Resize(sender As Object, e As EventArgs) Handles Me.Resize
        If Me.Height > 300 Then
            ...
            dgv_MasterEDataRegistration.Top = 66
            dgv_MasterEDataRegistration.Height = Me.Height - 180
    ...
    Private Sub btn_ExportToExcel_Click(sender As Object, e As EventArgs) Handles btn_ExportToExcel.Click
        Try
            ...
            For j = 0 To dgv_MasterEDataRegistration.Columns.Count - 1

                excelworksheet.Cells(1, j + 1) = dgv_MasterEDataRegistration.Columns(j).HeaderText
                For i = 1 To dgv_MasterEDataRegistration.Rows.Count - 1
                    '
                    excelworksheet.Cells(i + 1, j + 1) = "'" & dgv_MasterEDataRegistration(j, i - 1).Value.ToString
                Next
    ...
    Private Sub txt_CariData_TextChanged(sender As Object, e As EventArgs) Handles txt_CariData.TextChanged
        Try
            rs = New ADODB.Recordset
            search = "select * from [AN_SUMATRA].[dbo].[TM_tb_Employees] where [employee_description] like '%" & txt_CariData.Text & "%' order by [id]"
            With rs
                .CursorLocation = CursorLocationEnum.adUseClient
            ...
            Call tampilgrid()
            If rs.BOF Then
                dgv_MasterEDataRegistration.DataSource = rs
                dgv_MasterEDataRegistration.Refresh()
            End If
