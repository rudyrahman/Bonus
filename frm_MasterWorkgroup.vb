...
Public Class frm_MasterWorkgroup
    ...
    Private Sub frm_MasterWorkgroup_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        If (dgv_MasterWorkgroup.Rows.Count > 0) Then
            dgv_MasterWorkgroup.Rows(0).Selected = True
        End If
    End Sub
    ...
    Private Sub AutoNumberRowsForGridView()
        If dgv_MasterWorkgroup IsNot Nothing Then
            Dim count As Integer = 0
            While (count <= (dgv_MasterWorkgroup.Rows.Count - 2))
                dgv_MasterWorkgroup.Item(0, count).Value = String.Format((count + 1).ToString(), "0")
                count += 1
            End While
        End If
    End Sub
